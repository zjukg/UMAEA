...
from .Tool_model import GAT, GCN, VAE, GDiffusionModel
import pdb
...
class VirEmbGen(nn.Module):
    def __init__(self, args, modal_num=3):
        ...
        # ent，rel，attr，surface*2
        self.dim = [self.input_dim, self.args.attr_dim, self.args.attr_dim, self.args.char_dim, self.args.name_dim]
        self.trans_fc = nn.Linear(sum(self.dim[:modal_num]), self.args.img_dim)
...
class VirEmbGen_vae(nn.Module):
    def __init__(self, args, modal_num=3):
        ...
        self.dim = [self.input_dim, self.args.attr_dim, self.args.attr_dim, self.args.char_dim, self.args.name_dim]
        # V3
        hidden_list = [self.args.img_dim]
        # V4
        # hidden_list = []
        # V5
        # hidden_list = [self.args.img_dim * 2]
        self.vae = VAE(sum(self.dim[:modal_num]), hidden_list, self.args.img_dim)
        # self.GD = GDiffusionModel(sum(self.dim[:modal_num]), sum(self.dim[:modal_num]))
        self.trans_fc = nn.Linear(self.args.img_dim, self.args.img_dim)
    ...
    def forward(self, embs):
        ...
        hidden, hidden_norm, x_hat, kl_div = self.vae(hyb_emb)

        return F.normalize(hidden), F.normalize(hidden_norm), F.normalize(x_hat), hyb_emb, kl_div
...
class MultiModalEncoder(nn.Module):
    ...
    def __init__(self, args,
        ...
        # self.vir_emb_gen = VirEmbGen(args=self.args, modal_num=self.args.modal_num)
    ...
    def forward(self,
        ...
        if self.args.stage >= 1:
            if self.args.adapt == "mlp":
                vir_emb = self.vir_emb_gen([gph_emb, rel_emb, att_emb, name_emb, char_emb])

            else:
                vir_emb, vir_emb_norm, x_hat, hyb_emb, kl_div = self.vir_emb_gen_vae([gph_emb, rel_emb, att_emb, name_emb, char_emb])

            if ent_wo_img.shape[0] > 0:
                if not _test:
                    img_emb[ent_wo_img, :] = vir_emb[ent_wo_img, :]
                else:
                    img_emb[ent_wo_img, :] = vir_emb_norm[ent_wo_img, :]
